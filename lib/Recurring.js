...
Recurring.prototype._setXML = function(method, fn) {
	...
	switch (method) {
		...
		case 'recurring.update':
			...
			var lines = recurring.node('lines')
				...
				if (typeof this[key] !== 'function') {
					switch (key) {
						...
						case 'autobill':
							...
							var card = autobill.node('card')
							var expiration = card.node('expiration')
		...
		case 'recurring.delete':
			request.node('recurring_id').text(this.recurring_id)
			break
...
Recurring.prototype._getXML = function(xml, fn) {
	var nodes = xml.get('//xmlns:recurring', this.freshbooks.ns).childNodes()
	...
	this._setXML('recurring.list', options, xml => {
		this.freshbooks._get(xml, (err, xml) => {
			if (err !== null) {
				...
				var recurrings = xml.get('//xmlns:recurrings', this.freshbooks.ns)
				var options = {
					page: recurrings.attr('page').value(),
				...
				xml.find('//xmlns:recurring', this.freshbooks.ns).forEach(a => {
