...
Payment.prototype._setXML = function (method, fn) {
  ...
  if (method == 'payment.create' || method == 'payment.update') { var payment = request.node('payment') }

  switch (method) {
    case 'payment.create':
      if (this.client_id) { payment.node('client_id').text(this.client_id) }
      if (this.invoice_id) { payment.node('invoice_id').text(this.invoice_id) }
