...
Tax.prototype._setXML = function(method, fn) {
	...
	switch (method) {
		...
		case 'tax.get':
		case 'tax.delete':
			request.node('tax_id').text(this.tax_id)
			break
	...
	this._setXML('tax.create', xml => {
		this.freshbooks._get(xml, (err, xml) => {
			if (err !== null) {
				...
				this.tax_id = xml.get('//xmlns:tax_id', this.freshbooks.ns).text()
	...
	this._setXML('tax.list', options, xml => {
		this.freshbooks._get(xml, (err, xml) => {
			if (err !== null) {
				...
				var taxes = xml.get('//xmlns:taxes', this.freshbooks.ns)
				var options = {
				...
				xml.find('//xmlns:tax', this.freshbooks.ns).forEach(a => {
