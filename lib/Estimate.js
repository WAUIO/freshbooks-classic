...
var parser = string => {
	var x = {}
	xml2js.parseString(string, {trim: true, explicitRoot: false, emptyTag: undefined, explicitArray: false, ignoreAttrs: true, async: false}, (err, result) => {
		if(err){throw err}
		x = result
	})
	return x
...
Estimate.prototype._getXML = function(xml, fn) {
	...
	console.log(JSON.stringify(parser(nodes.toString(), null, 4)))
